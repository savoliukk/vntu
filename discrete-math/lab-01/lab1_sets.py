...
Завдання 1
U = {format_set(U_lab, letters=True)}
A = {format_set(A_lab, letters=True)}
B = {format_set(B_lab, letters=True)}
C = {format_set(C_lab, letters=True)}
D = {format_set(D_lab, letters=True)}
A ∩ B = {format_set(lab_ab, letters=True)}
(A ∩ B) ∪ C = {format_set(lab_ab_union_c, letters=True)}
X = ((A ∩ B) ∪ C) ∪ D = {format_set(lab_x, letters=True)}
...
C = {format_set(C_pr)}
D = {format_set(D_pr)}
A ∪ B = {format_set(pr_a_union_b)}
~C = U \\ C = {format_set(pr_not_c)}
~D = U \\ D = {format_set(pr_not_d)}
~C ∩ ~D = {format_set(pr_not_c_and_not_d)}
X = (A ∪ B) ∪ (~C ∩ ~D) = {format_set(pr_x)}
...
A ∩ B ∩ C = {regions["abc"]}
Принаймні один додаток = {at_least_one}
Лише один конкретний додаток = {only_one}
